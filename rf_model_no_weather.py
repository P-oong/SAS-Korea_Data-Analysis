...
# 모듈 디렉토리 추가 (model_optimizer 임포트를 위해)
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '..')))
from modules.model_optimizer import ModelOptimizer, rmse_scorer
...
def train_model_with_gridsearch(X_train, y_train, cv=5, n_jobs=-1):
    """RandomForest 모델을 그리드서치로 최적화하여 학습"""
    ...
    # 커스텀 파라미터 그리드 설정
    optimizer.param_grid = custom_param_grid
    ...
    logger.info(f"최적 파라미터: {optimizer.best_params}")
    
    # 최적화 결과 출력
    optimizer.print_optimization_results()
    
    # 특성 중요도 계산
    feature_importance = optimizer.get_feature_importance(feature_names=X_train.columns)
    ...
    # 최적 모델 반환
    return best_model, feature_importance, optimizer.best_params
