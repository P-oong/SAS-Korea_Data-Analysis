...
def train_model(X_train, y_train, categorical_features=None):
    """CatBoost 모델 학습 및 k-fold 교차 검증""" # 함수 설명 변경
    logger.info("CatBoost 모델 학습 및 k-fold 교차 검증 시작") # 로그 메시지 변경
    
    # 지정된 하이퍼파라미터 적용 (CatBoost 파라미터 이름 사용)
    params = {
        'iterations': 200,       # n_estimators -> iterations
        'depth': 8,              # max_depth -> depth
        'learning_rate': 0.2,
        'l2_leaf_reg': 1,        # L2 정규화
        'subsample': 0.8,        # 행 샘플링 비율
        'rsm': 1.0,              # colsample_bytree -> rsm (특성 샘플링 비율)
        'random_state': 42,
        'verbose': 50,           # 학습 로그 출력 빈도 (50 iteration 마다)
        'loss_function': 'RMSE' # 회귀 문제 명시
    }
    
    logger.info(f"사용할 파라미터: {params}")
    
    # 모델 정의 (CatBoostRegressor 사용)
    # 모든 범주형 변수는 이미 타겟 인코딩 되었으므로 cat_features 지정 불필요
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(X_train), 1):
        ...
        # Fold별 모델 학습 (CatBoost 사용)
        fold_model = CatBoostRegressor(**params)
        # K-Fold 내부에서는 로그를 끄기 위해 verbose=False 설정 가능
    ...
    # 특성 중요도 계산 (CatBoost는 feature_importances_ 속성 제공)
...
def save_model_data(model, preprocess_info, model_name='CatBoost_no_weather'): # 모델 이름 변경
    """모델 저장 (CatBoost, 날씨 제외 버전)"""
    ...
    # CatBoost 모델은 joblib 대신 자체 save_model 사용 권장
    # model.save_model(model_path, format="cbm") # 필요시 주석 해제 및 joblib 제거
    joblib.dump(model_data, model_path) # 일단 joblib 유지
    logger.info(f"모델이 저장되었습니다: {model_path}")
    
    return model_path

def save_predictions_data(test_df, predictions, model_name='CatBoost_no_weather'): # 모델 이름 변경
    """예측 결과 저장 (CatBoost, 날씨 제외 버전)"""
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
